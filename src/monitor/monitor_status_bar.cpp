...
void debug_status_bar_refresh()
{
    ...
    // registers
    pc.m68k->dump_registers(help_string);
    ...
    for(int i=0; i<(debug_console.status_bar_rows - 10); i++ )
    {
        ...
        if( debug_console.status_bar_hex_view == true )
        {
            ...
            {
                snprintf(help_string_2, 256, "%04x ", pc.mmu->read_memory_16(temp_pc + (2*i) ) );
    ...
    // vicv
    snprintf(help_string, 256, "    pixel: %3u\n scanline: %3u\n   hblank: %3u\n   vblank: %3u", pc.vicv->get_current_pixel(), pc.vicv->get_current_scanline(), pc.vicv->is_hblank() ? 1 : 0, pc.vicv->is_vblank() ? 1 : 0 );
